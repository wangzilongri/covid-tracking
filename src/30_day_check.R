...
# construct the 30-day table

latest_date = max(county_data$days_from_start)
county_30data<-subset(county_data, days_from_start > latest_date-30 & days_from_start <= latest_date)
county_30data <- county_30data[, which(names(county_30data) %in% c("date","fips", "cases"))]

county_30data <- reshape(county_30data, 
             timevar = "date",
             idvar = c("fips"),
             direction = "wide")

for (fips in fips_list){
  fips.df<-county_30data[which(county_30data$fips==fips),2:31]
  names(fips.df)<-NULL
  fips.list<-unlist(fips.df)
  row_diff<-diff(fips.list) 
  dtest<-min(row_diff)
  county_30data[which(county_30data$fips==fips),"d20"]<-as.numeric(isTRUE(dtest< -20))
}

fwrite(county_30data,"../data/30_Day_Check.csv",row.names=FALSE)

final<-county_30data[, which(names(county_30data) %in% c("fips", "d20"))]

fwrite(final,"../data/30_Day_Table.csv",row.names=FALSE)

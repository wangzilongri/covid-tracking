...
print(paste0("TLGRF_State_Forests.R latest_date is ",toString(latest_date)))


mainDir = "../data/output"
subDir = paste("backtest_LLF_TLGRF_state_forests_windowsize=",toString(windowsize),sep="")
...
#for(cutoff in cutoff.list){
  ...
  try({
    ...
    X_test_columns <- colnames(covariates.test.unique)
    ...
    state.tau.hat <- predict(state.tau.forest, covariates.test.unique, linear.correction.variables=valid_indices, estimate.variance = TRUE)$predictions
    ...
    output.fname = paste("LLF_TLGRF_block_results_",toString(cutoff),".csv",sep="")

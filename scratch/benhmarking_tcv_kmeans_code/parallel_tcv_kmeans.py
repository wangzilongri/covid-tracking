...
def process_window(df, starting_day, window_size, max_window=14, n_days_to_validate=14):
...
def feed_parallel_tcv(mask, df):
    return parallel_tcv(df[mask], max_window=14, n_days_to_validate=14)
...
if True:
    K = int(sys.argv[1])
    k = int(sys.argv[2])
    cluster_tcv_dict = serial_clustered_tcv_by_Kk(test_df, K, k)
    os.makedirs("./kmeans_tcv_results", exist_ok=True)
    with open(os.path.join("./kmeans_tcv_results","cluster_tcv_dict_key=({},{}).pickle".format(K,k)), 'wb') as f:

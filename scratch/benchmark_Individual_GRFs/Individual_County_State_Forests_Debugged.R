...
subDir = "individual_county_backtest_by_county"
...
#for(cutoff in cutoff.list){
  output_subfolder = file.path(outputfolder, toString(fips_code))
  ...
  check.file.name <- paste0("grf_individual_county_fips=",toString(fips_code),"_cutoff=",toString(cutoff),".rds")
  check.file.full.name <- file.path(fips_grf_subfolder_path, check.file.name) 
  ...
  tryCatch(expr={
	...
	covariates.test <- test_df[,-which(names(test_df) %in% exclusion.test)]
	...
	destfolder = file.path(output_subfolder,output.fname)

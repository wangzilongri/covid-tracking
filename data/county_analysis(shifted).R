...
county_analysis <- function(state, county_data, cutoffstart,cutoffend, predictionsize){
  ...
  for (fips in restricted_state_fips_list){
    ...
    restricted_state_df[which(restricted_state_df$fips == fips), grf.predict.string] <- predictionsize*r.grf + latest_cases
    restricted_state_df[which(restricted_state_df$fips == fips), "predicted.grf.augmented"] <-predictionsize*augmented.r.grf + latest_cases
    restricted_state_df[which(restricted_state_df$fips == fips), "predicted.grf.fonly"] <-predictionsize*fonly.r.grf + latest_cases

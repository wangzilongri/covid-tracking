...
source("../src/county_analysis_lm.R")
...
mainDir = "../data"
subDir = paste("experimental_block_windowsize=",toString(windowsize+1),sep="")
...
foreach(cutoff = cutofflist) %dopar%{
  ...
  # Taking second day data within the block
  Tfirst<-subset(restricted_state_df, days_from_start ==first+1)

...
grf.subfolder = paste("time_invariant_grf_windowsize=",toString(windowsize),"_numtrees=",toString(num_trees),sep="")
...
#for(cutoff in (cutoff_list)){
    ...
    backtest.check.file.name <- paste0("time_invariant_grf_block_results_", toString(cutoff), ".csv")
    ...
    try({ # START TRY
        ...
        case_number_columns <- c("fips","date","county","state","days_from_start","log_rolled_cases", "days_from_start")

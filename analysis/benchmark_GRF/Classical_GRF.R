...
registerDoParallel(cores=min(detectCores(), 40))
...
mainDir = "./classical_grf_results"
dir.create(mainDir, showWarnings = FALSE)

subDir = paste("classical_grf_backtest_state_forests_windowsize=",toString(windowsize),sep="")
outputfolder = file.path(mainDir, subDir)


dir.create(outputfolder, showWarnings = FALSE)

grf.subfolder = paste("classical_grf_windowsize=",toString(windowsize),"_numtrees=",toString(num_trees),sep="")
...
#for(cutoff in cutoff.list){
  ...
  try({
    ...
    output.fname = paste("classical_grf_block_results_",toString(cutoff),".csv",sep="")

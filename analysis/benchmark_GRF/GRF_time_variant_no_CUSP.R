...
mainDir = "./time_variant_no_CUSP_grf_results"
dir.create(mainDir, showWarnings = FALSE)

subDir = paste("time_variant_no_CUSP_grf_backtest_state_forests_windowsize=",toString(windowsize),"_numtrees=",toString(num_trees),sep="")
outputfolder = file.path(mainDir, subDir)


dir.create(outputfolder, showWarnings = FALSE)

grf.subfolder = paste("time_variant_no_CUSP_grf_windowsize=",toString(windowsize),"_numtrees=",toString(num_trees),sep="")
...
print(paste0("Beginning time variant from ", toString(start_day), " to ", toString(end_day)))

foreach(cutoff = (cutoff_list)) %dopar%{
#for(cutoff in (cutoff_list)){
    # Check if result already exists
    check.file.name <- paste0("time_variant_no_CUSP_grf_stateforest_cutoff=", toString(cutoff), ".rds")
    check.file.full.name <- file.path(grf.outputfolder, check.file.name)
    
    backtest.check.file.name <- paste0("time_variant_no_CUSP_grf_block_results_", toString(cutoff), ".csv")
    ...
    try({ # START TRY
        ...
        # Index of CUSP columns
        X <- X[, -CUSP_cols_indices]
        ...
        X_test <- X_test %>%
          select_if(is.numeric)
        X_test <- X_test[, -CUSP_cols_indices]
        ...
        output.fname = paste("time_variant_no_CUSP_grf_block_results_",toString(cutoff),".csv",sep="")

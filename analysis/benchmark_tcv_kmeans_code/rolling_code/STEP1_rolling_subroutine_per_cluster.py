...
def rolling_subroutine(df, fips_list, window_sizes=list(range(2,15)), shift_list=[7], history=7):
    ...
    for window_size in window_sizes:
        ...
        daily_metrics_df["rolling={}_mae_wsize={}".format(history, window_size)] = daily_metrics_df["mae_wsize={}".format(window_size)].rolling(history).sum()
        daily_metrics_df["rolling={}_rmse_wsize={}".format(history, window_size)] = daily_metrics_df["rmse_wsize={}".format(window_size)].rolling(history).sum()
    best_wsizes = pd.DataFrame(daily_metrics_df["date"])
    daily_mae_df = daily_metrics_df[["rolling={}_mae_wsize={}".format(history, window_size) for window_size in window_sizes]]
    best_wsizes["best_mae_window"] = daily_mae_df.idxmin(axis=1).str.split('=').str[2]
    
    daily_rmse_df = daily_metrics_df[["rolling={}_rmse_wsize={}".format(history, window_size) for window_size in window_sizes]]
...
def rolling_worker(all_beta_df_results_diff, kmeans_clusters_by_fips, Kk, window_sizes=list(range(2,15)), shift_list=[7], history=7, kmeans_tcv_rolling_directory="./kmeans_tcv_rolling"):
    K, k = Kk
    K_subfolder_directory = os.path.join(kmeans_tcv_rolling_directory,str(K))
    ...
    print("Executing rolling_worker on Cluster=({},{})".format(K,k))
    fips_list = generate_mask(kmeans_clusters_by_fips, K, k)
    best_wsizes = rolling_subroutine(all_beta_df_results_diff, fips_list, window_sizes=list(range(2,15)), shift_list=[7], history=7)
    best_wsizes["K"] = K
    best_wsizes["k"] = k
...
if __name__ == "__main__":
    ...
    print("Reading in all_beta_df_results_diff")
    ...
    os.makedirs(kmeans_tcv_rolling_directory, exist_ok=True)
    ...
    for K in K_list:
        ...
        with Parallel(n_jobs=-1, backend='multiprocessing') as parallel:
            best_wsizes_arr = parallel(delayed(rolling_worker)(all_beta_df_results_diff, kmeans_clusters_by_fips, Kk, window_sizes=window_sizes, shift_list=shift_list, history=history, kmeans_tcv_rolling_directory=kmeans_tcv_rolling_directory) for Kk in Kk_list)

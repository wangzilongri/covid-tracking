...
def test_metric_worker(df, items, t):
    ...
    rmse_fips_df = test_train_rmse
    test_fips_df = test_data
...
def test_time(hhs_clustered_panel_data, dfs_dict, cluster_key, max_window=14):
    ...
    with Parallel(n_jobs=-1, backend='multiprocessing') as parallel:
        metrics_samples_tuple_arr = parallel(delayed(test_metric_worker)(df, items, t) for t in tqdm(days))    
...
if __name__ == "__main__":
    ...
    os.makedirs("kmeans_tcv_1st_design", exist_ok=True)
    for i, Kk in tqdm(enumerate(available_Kk)):
        K, k = Kk
        print("Generating 1st design for {}, which is {}th out of {}".format(Kk, i+1, len(available_Kk)))
        metrics_df = test_time(hhs_clustered_panel_data, dfs_dict, Kk)
        metrics_df_fname = os.path.join("kmeans_tcv_1st_design", "metrics_df_({},{}).csv".format(K,k))

...
def cumsum_subroutine(df, fips_list, window_sizes=list(range(2,15)), shift_list=[7]):
    ...
    daily_mae_df = daily_metrics_df[["cumsum_mae_wsize={}".format(window_size) for window_size in window_sizes]]
    best_wsizes["best_mae_window"] = daily_mae_df.idxmin(axis=1).str.split('=').str[1]
    
    daily_rmse_df = daily_metrics_df[["cumsum_rmse_wsize={}".format(window_size) for window_size in window_sizes]]
...
def cumsum_worker(all_beta_df_results_diff, kmeans_clusters_by_fips, Kk, window_sizes=list(range(2,15)), shift_list=[7], kmeans_tcv_cumsum_directory="./kmeans_tcv_cumsum"):
    K, k = Kk
    K_subfolder_directory = os.path.join(kmeans_tcv_cumsum_directory,str(K))
    ...
    print("Executing cumsum_worker on Cluster=({},{})".format(K,k))
    fips_list = generate_mask(kmeans_clusters_by_fips, K, k)
    best_wsizes = cumsum_subroutine(all_beta_df_results_diff, fips_list, window_sizes=list(range(2,15)), shift_list=[7])
    best_wsizes["K"] = K
    best_wsizes["k"] = k
...
if __name__ == "__main__":
    ...
    kmeans_tcv_cumsum_directory = "./kmeans_tcv_cumsum"
    os.makedirs(kmeans_tcv_cumsum_directory, exist_ok=True)
    ...
    for K in K_list:
        ...
        with Parallel(n_jobs=-1, backend='multiprocessing') as parallel:
            best_wsizes_arr = parallel(delayed(cumsum_worker)(all_beta_df_results_diff, kmeans_clusters_by_fips, Kk, window_sizes=window_sizes, shift_list=shift_list, kmeans_tcv_cumsum_directory=kmeans_tcv_cumsum_directory) for Kk in Kk_list)

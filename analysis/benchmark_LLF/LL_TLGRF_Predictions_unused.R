...
TLGRF_RDS_directory <- "../../data/output/TLGRF_windowsize=2_numtrees=200"

mainDir = "./LLF_TLGRF_results"
dir.create(mainDir, showWarnings = FALSE)


outputfolder = paste("llf_TLGRF_windowsize=",toString(windowsize),"_numtrees=",toString(num_trees),sep="")
grf.outputfolder = file.path(mainDir,outputfolder)
...
for (lambda_exponent in lambda_exponent_list){
    ...
    check.file.full.name <- file.path(TLGRF_RDS_directory, check.file.name)
    
    backtest.check.file.name <- paste0("llf_TLGRF_results", toString(cutoff),"_lambda_exponent_",toString(lambda_exponent), ".csv")
    backtest.check.file.full.name <- file.path(grf.outputfolder, backtest.check.file.name)
    ...
    print(paste0("Generate LL Prediction from TLGRF ", toString(cutoff), " lambda_exponent=", toString(lambda_exponent) ))
    try({ # START TRY
        start_time <- Sys.time()     

# Trains the Regression Forests with Blocked Data (first difference) for prediction
...
mainDir = "./regression_forest_blocked"
...
grf.subfolder = paste("windowsize=",toString(windowsize),"_numtrees=",toString(num_trees),sep="")
...
print(paste0("Beginning blocked regression forest from ", toString(start_day), " to ", toString(end_day)))

foreach(cutoff = (cutoff_list)) %dopar%{
#for(cutoff in (cutoff_list)){
    # Check if result already exists
    check.file.name <- paste0("regression_forest_block_cutoff=", toString(cutoff), ".rds")
    check.file.full.name <- file.path(grf.outputfolder, check.file.name)
    
    backtest.check.file.name <- paste0("regression_forest_block_results_", toString(cutoff), ".csv")
    ...
    print(paste0("Computing blocked regression forest for ", toString(cutoff)))
    try({ # START TRY
        start_time <- Sys.time()     
        ...
        if (file.exists(check.file.full.name)){
            ...
            rf <- regression_forest(X,Y, num.trees=num_trees)
            print(paste0("Saving ", check.file.full.name))
            saveRDS(rf, check.file.full.name)
        }
        ...
        r_LLF <- predict(rf, X_test, estimate.variance=TRUE)

...
beta_source = pl.read_csv("~/experimental-COVID-tracking/analysis/benchmark_GRF/time_invariant_GRF_results_df.csv")
...
def init_params(fips: int, T_interval: np.ndarray):
    ...
    # Obtain beta_tc
    test_rtc = fips_df.select(['days_from_start','r_GRF']).to_pandas().values
...
def process_and_save_results(results_subset):
    ...
    for (fips, T_interval), data in best_results.items():
        ...
        result_file_path = os.path.join(dir_path, f"grf_results_{T_interval[0]}_{T_interval[1]}.csv")
        ...
        metadata_file_path = os.path.join(dir_path, f"grf_metadata_{T_interval[0]}_{T_interval[1]}.txt")
...
if __name__ == "__main__":
    ...
    for fips in tqdm(fips_list[fips_list_start:fips_list_end]):
        ...
        try:
            ...
            #joblib_file_path = f"./dump/{fips}/results_grf.joblib"
            ...
            tqdm.write("Error for fips={}".format(fips))

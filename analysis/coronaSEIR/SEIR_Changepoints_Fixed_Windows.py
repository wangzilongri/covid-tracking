...
fixed_source_schema = {
    ...
    'log_rolled_cases': pl.Float64,
    'shifted_log_rolled_cases': pl.Float64,
    'beta_wsize=2': pl.Float64
}

fixed_source = pl.read_csv("~/experimental-COVID-tracking/analysis/benchmark_fixed_window/Fixed_windows_all_beta.csv").select(['fips','days_from_start','beta_wsize=2'])

beta_source = fixed_source.join(benchmark_colorado_dataset, on=["fips","days_from_start"], how="inner").sort(["fips","days_from_start"])
...
def init_params(fips: int, T_interval: np.ndarray):
    ...
    fips_df = beta_source.filter(pl.col("fips")==fips)
    ...
    # Obtain beta_tc
    test_rtc = fips_df.select(['days_from_start','beta_wsize=2']).with_columns(
        pl.col("beta_wsize=2")
        .fill_null(strategy="backward")  # Backfill initial NaNs
        .fill_null(strategy="forward")  # Forward fill remaining NaNs
    ).to_pandas().values
    test_rolled_cases = fips_df.select(['days_from_start','log_rolled_cases']).to_pandas().values
    test_rolled_cases[:,1] = np.exp(test_rolled_cases[:,1])
...
def process_and_save_results(results_subset):
    ...
    for (fips, T_interval), data in best_results.items():
        ...
        result_file_path = os.path.join(dir_path, f"wsize=2_results_{T_interval[0]}_{T_interval[1]}.csv")
        ...
        metadata_file_path = os.path.join(dir_path, f"wsize=2_metadata_{T_interval[0]}_{T_interval[1]}.txt")

...
tcv_source = pl.read_csv("tcv_beta.csv", schema=tcv_source_schema)

tcv_source = tcv_source.join(benchmark_colorado_dataset, on=["fips","days_from_start"], how="inner").sort(["fips","days_from_start"])
...
def init_params(fips: int, T_interval: np.ndarray):
    ...
    fips_df = tcv_source.filter(pl.col("fips")==fips)
    ...
    # Obtain beta_tc
    test_rtc = fips_df.select(['days_from_start','r_tcv']).to_pandas().values
...
def process_and_save_results(results_subset):
    ...
    for (fips, T_interval), data in best_results.items():
        ...
        result_file_path = os.path.join(dir_path, f"tcv_results_{T_interval[0]}_{T_interval[1]}.csv")
        ...
        metadata_file_path = os.path.join(dir_path, f"tcv_metadata_{T_interval[0]}_{T_interval[1]}.txt")

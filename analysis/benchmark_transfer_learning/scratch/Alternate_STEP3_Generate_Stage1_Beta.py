...
os.makedirs("betas", exist_ok=True)
...
def backup_exists(cutoff):
    filename = f"betas/beta_cutoff={cutoff}.npy"
...
def train_ols(index, even_df, odd_df):
    ...
    #beta = model.coef_
    coef, intercept = ridge_regression(X_np, y_np, alpha=0, return_intercept=True)
    beta = np.array(list(coef) + [intercept])
